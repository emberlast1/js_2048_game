...
function updateUI(curGame) {
    ...
    if (curGame.getStatus() === "win") messageWin.classList.remove("hidden");
    else if (curGame.getStatus() === "lose") messageLose.classList.remove("hidden");
    else {
 ...
 */ class Game {
    ...
    addRandomTile() {
        ...
        if (emptyTiles.length > 0) {
            const { row, cell } = emptyTiles[Math.floor(Math.random() * emptyTiles.length)];
